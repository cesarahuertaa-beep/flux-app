...
// There are TWO instances of `{/* Modal preview */}` in the file now (or maybe just one).
// I will just split by `{/* Modal preview */}` and KEEP only the part before the FIRST occurrence!
// Because the FIRST occurrence is inside TimerCard, and everything after that up to `}` of TimerCard is the modal.

// Let's find exactly the end of TimerCard.
// TimerCard ends with:
//         </button>
//       </div>
//     </div>
...
// I will just replace the entire TimerCard component!
const timerCardRegex = /function TimerCard\(\{ brandColor \}\) \{[\s\S]*?\}\s*export default function Training/m;

const correctTimerCard = `function TimerCard({ brandColor }) {
...
content = content.replace(timerCardRegex, correctTimerCard);

// Now TimerCard is perfectly clean!
// Does the file still have the modal at the bottom?
// Let's check how many times `{/* Modal preview */}` exists.
let parts = content.split('{/* Modal preview */}');
if (parts.length === 1) {
  // We need to inject it at the bottom.
  const modalStr = `
      {/* Modal preview */}
      {previewEx && (
        <div 
...
console.log("TimerCard hard-replaced");
