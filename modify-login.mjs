...
// We need to inject { suscripcion: 'estandar' } into the dbPost inside signUpSubmit
content = content.replace(
  /nutriologo_id: null/g,
  "nutriologo_id: null, suscripcion: 'estandar'"
);
...
content = content.replace(
  'mode === "signup" ? \'bg-white shadow-sm text-[var(--brand-primary)]\' : \'text-[#6B7A8D]',
  '(mode.startsWith("signup")) ? \'bg-white shadow-sm text-[var(--brand-primary)]\' : \'text-[#6B7A8D]'
);
content = content.replace(
...
const oldSignupStart = '{/* 🚀 SIGNUP FORM 🚀 */}';
const oldSignupEnd = '{/* 🔄 RESET FORM 🔄 */}';
const startIndex = content.indexOf(oldSignupStart);
const endIndex = content.indexOf(oldSignupEnd);
...
if (startIndex !== -1 && endIndex !== -1) {
  const newSignupBlocks = `
        ...
        {mode === "signup_type" && <>
          ...
          <div className="flex flex-col gap-3 mb-4">
            ...
            <button onClick={() => { setSignupType("nutriologo_estudiante"); setMode("signup_pro"); }} className="p-4 bg-white border border-[#E2E8F0] rounded-2xl flex items-center gap-4 hover:border-amber-500 hover:shadow-md transition-all text-left">
              ...
              <div>
                <p className="text-sm font-bold text-[#0B1929]">Nutriólogo Estudiante</p>
        ...
        {mode === "signup_pro" && <>
          <div className="mb-6 text-center">
            <h3 className="text-lg font-bold text-[#0B1929] mb-1">{signupType === 'nutriologo' ? 'Solicitud Nutriólogo' : 'Solicitud Estudiante'}</h3>
...
`;
  ...
  content = content.substring(0, startIndex) + newSignupBlocks + '\n        ' + content.substring(endIndex);
